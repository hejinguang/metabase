...
(defn- ag:doubleSum [field output-name] {:type :doubleSum, :name output-name, :fieldName (->rvalue field)})
...
(defn- parse-filter-subclause:filter [{:keys [filter-type field value] :as filter}]
  ...
  (when-not (instance? DateTimeField field)
    (if (= (dimension-or-metric? field) :metric)
      (log/warn (u/format-color 'red "WARNING: Filtering only works on dimensions! '%s' is a metric. Ignoring %s filter." (->rvalue field) filter-type))
      (let [value (->rvalue value)]
        (case filter-type
          :inside      (let [lat (:lat filter)
                             lon (:lon filter)]
                         {:type   :and
                          :fields [(filter:js (:field lat) "function (x) { return x >= %s && x <= %s; }" (num (->rvalue (:min lat))) (num (->rvalue (:max lat))))
                                   (filter:js (:field lon) "function (x) { return x >= %s && x <= %s; }" (num (->rvalue (:min lon))) (num (->rvalue (:max lon))))]})
